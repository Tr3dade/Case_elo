...
def _canais_proprios(caminho_vendas: str) -> pd.DataFrame:
    """Vendas de todos os canais EXCETO o Marketplace, sem linhas com dado faltando."""
    vendas = pd.read_csv(caminho_vendas).dropna(subset=COLUNAS_CANAIS_PROPRIOS)
...
def preparar(caminho_vendas: str, caminho_saida: str) -> pd.DataFrame:
    ...
    mkt = vendas[vendas["canal"] == CANAL_MARKETPLACE][["order_id", "receita_bruta", "receita_liquida", "custo_frete"]]
...
def preparar_rampa(caminho_vendas: str, caminho_saida: str) -> pd.DataFrame:
    ...
    """
    proprios = _canais_proprios(caminho_vendas)
...
def preparar_perfil(caminho_vendas: str, caminho_saida: str) -> pd.DataFrame:
    ...
    proprios = _canais_proprios(caminho_vendas)
    por_canal = proprios.groupby("canal")
    perfil = pd.DataFrame({
