"""Testes do ui/qualidade_dados.py: os números das 6 ressalvas e as frases que os mostram.
...
def dados():
    vendas = pd.DataFrame({
        ...
        "status_pagamento": ["Aprovado", "Cancelado", "Aprovado", "Aprovado", "Aprovado", "Aprovado"],
...
def test_calcular_receita_atribuida_e_bases_de_cada_bloco(dados):
    ...
    assert (r["mensal_pedidos"], r["mensal_receita"]) == (5, 1200.0)                    # O1, O2, O3, O5, O6: 2023, todos os status
...
def numeros_de_mentira():
    ...
    return {
        ...
        "mensal_pedidos": 26538, "mensal_receita": 18_086_795.0,
...
def test_sao_seis_frases_com_numeros_no_formato_brasileiro():
    ...
    esperado = ["01/01/2023 a 26/01/2024", "Só 346 dos 15.000 clientes do cadastro (2,3%)",
                ...
                "3 maiores IDs concentram 67,0% dos pedidos", "20.792 pedidos; R$ 14,2 mi", "24.437 pedidos; R$ 16,7 mi",
                "26.538 pedidos; R$ 18,1 mi", "27.759 pedidos; R$ 18,9 mi", "Marketplace 6.040 pedidos; R$ 4,0 mi"]
    tela = " ".join(textos)
...
def test_texto_e_sobrio_e_diz_que_as_bases_nao_reconciliam():
    ...
    assert tela.endswith("Os números dos blocos não são diretamente comparáveis.")   # a conclusão do item 6
...
def test_real_numeros_da_auditoria(reais):
    ...
    assert (r["kpis_pedidos"], r["roas_pedidos"], r["mensal_pedidos"], r["pedidos_total"]) == (20792, 24437, 26538, 27759)
    assert r["kpis_pedidos"] == r["roas_pedidos"] - 3645                                # 3.645 devolvidos entre os não cancelados
    assert (r["kpis_receita"], r["receita_real"], r["mensal_receita"], r["simulador_receita"]) == pytest.approx(
        (14_170_454.54, 16_669_816.75, 18_086_795.16, 18_889_334.01), abs=0.01)
